...
void showUI(Service& serv, RepoFile& r) {
	int op;
	...
	while (val) {
		...
		case 1: {if (parked == X) cout << "The parking is full!" << endl;
			...
			if (addUI(serv) == 2) { parked++; left--; r.saveToFile();
			}
		}r.saveToFile();
			  break; }
		case 2: {if (deleteUI(serv) == 1) parked--; left++; r.saveToFile(); break; }
		case 3: {updateUI(serv); r.saveToFile(); break; }
